# -*- coding: utf-8 -*-
"""
生产级数据工程模块

提供多模态数据处理能力：
- 数据去重（Perceptual Hash / MinHash / SimHash）
- 噪声过滤（NSFW、错配、质量评估）
- 模态一致性校验
- 版权与合规扫描
- 数据增强（图像、文本、音频）
- 质量评分和统计分析
"""

import logging
import time
from typing import Dict, Any, Optional, List, Tuple, Set, Iterator, Callable, Union
from dataclasses import dataclass, field, asdict
from abc import ABC, abstractmethod
from enum import Enum
from collections import defaultdict, deque, Counter
import hashlib
import json
import os
import pickle
import random

import torch
import numpy as np

from .multimodal_config import (
    DataEngineeringConfig,
    DataDeduplicationConfig,
    DataFilterConfig,
    DataAugmentationConfig,
    DataSourceType,
    ModalityType
)

logger = logging.getLogger(__name__)


# ==================== 枚举和数据类 ====================

class QualityLevel(Enum):
    """数据质量等级"""
    EXCELLENT = "excellent"   # >= 0.9
    GOOD = "good"             # >= 0.7
    FAIR = "fair"             # >= 0.5
    POOR = "poor"             # < 0.5


class ProcessingStage(Enum):
    """处理阶段"""
    RAW = "raw"
    DEDUPLICATED = "deduplicated"
    FILTERED = "filtered"
    AUGMENTED = "augmented"


@dataclass
class DataQualityMetrics:
    """数据质量指标"""
    total_samples: int = 0
    valid_samples: int = 0
    duplicate_samples: int = 0
    filtered_samples: int = 0
    
    # 质量分布
    excellent_count: int = 0
    good_count: int = 0
    fair_count: int = 0
    poor_count: int = 0
    
    # 问题统计
    issue_counts: Dict[str, int] = field(default_factory=dict)
    
    # 模态统计
    modality_counts: Dict[str, int] = field(default_factory=dict)
    
    # 平均指标
    avg_quality_score: float = 0.0
    avg_text_length: float = 0.0
    avg_image_size: Tuple[float, float] = (0.0, 0.0)
    
    def update_quality_distribution(self, score: float) -> None:
        """更新质量分布"""
        if score >= 0.9:
            self.excellent_count += 1
        elif score >= 0.7:
            self.good_count += 1
        elif score >= 0.5:
            self.fair_count += 1
        else:
            self.poor_count += 1
    
    def get_quality_distribution(self) -> Dict[str, float]:
        """获取质量分布百分比"""
        total = max(1, self.excellent_count + self.good_count + self.fair_count + self.poor_count)
        return {
            'excellent': self.excellent_count / total,
            'good': self.good_count / total,
            'fair': self.fair_count / total,
            'poor': self.poor_count / total,
        }


@dataclass
class ProcessingStatistics:
    """处理统计信息"""
    stage: ProcessingStage
    input_count: int = 0
    output_count: int = 0
    processing_time: float = 0.0
    throughput: float = 0.0  # samples/second
    
    error_count: int = 0
    errors: List[str] = field(default_factory=list)
    
    def compute_throughput(self) -> None:
        """计算吞吐量"""
        if self.processing_time > 0:
            self.throughput = self.input_count / self.processing_time


class DataStatisticsMonitor:
    """数据统计监控器"""
    
    def __init__(self, history_size: int = 10000):
        self.history_size = history_size
        self.quality_metrics = DataQualityMetrics()
        self.stage_stats: Dict[ProcessingStage, ProcessingStatistics] = {}
        
        # 历史记录
        self._quality_history: deque = deque(maxlen=history_size)
        self._sample_history: deque = deque(maxlen=1000)
        
    def record_sample(self, sample: 'MultiModalSample', stage: ProcessingStage) -> None:
        """记录样本"""
        self.quality_metrics.total_samples += 1
        
        if sample.is_valid:
            self.quality_metrics.valid_samples += 1
        
        if sample.is_duplicate:
            self.quality_metrics.duplicate_samples += 1
        
        # 记录质量分数
        self._quality_history.append(sample.quality_score)
        self.quality_metrics.update_quality_distribution(sample.quality_score)
        
        # 记录问题
        for issue in sample.issues:
            issue_type = issue.split(':')[0] if ':' in issue else issue
            self.quality_metrics.issue_counts[issue_type] = \
                self.quality_metrics.issue_counts.get(issue_type, 0) + 1
        
        # 记录模态
        for modality in sample.modalities.keys():
            self.quality_metrics.modality_counts[modality] = \
                self.quality_metrics.modality_counts.get(modality, 0) + 1
        
        # 更新平均值
        n = self.quality_metrics.total_samples
        self.quality_metrics.avg_quality_score = \
            (self.quality_metrics.avg_quality_score * (n-1) + sample.quality_score) / n
        
        # 保存样本历史
        self._sample_history.append({
            'sample_id': sample.sample_id,
            'quality_score': sample.quality_score,
            'is_valid': sample.is_valid,
            'stage': stage.value,
        })
    
    def record_stage(self, stats: ProcessingStatistics) -> None:
        """记录处理阶段统计"""
        self.stage_stats[stats.stage] = stats
    
    def get_quality_trend(self) -> str:
        """获取质量趋势"""
        if len(self._quality_history) < 100:
            return "insufficient_data"
        
        recent = list(self._quality_history)[-50:]
        older = list(self._quality_history)[-100:-50]
        
        recent_avg = sum(recent) / len(recent)
        older_avg = sum(older) / len(older)
        
        if recent_avg > older_avg * 1.05:
            return "improving"
        elif recent_avg < older_avg * 0.95:
            return "declining"
        return "stable"
    
    def get_summary(self) -> Dict[str, Any]:
        """获取摘要"""
        return {
            'total_samples': self.quality_metrics.total_samples,
            'valid_samples': self.quality_metrics.valid_samples,
            'duplicate_samples': self.quality_metrics.duplicate_samples,
            'valid_rate': self.quality_metrics.valid_samples / max(1, self.quality_metrics.total_samples),
            'duplicate_rate': self.quality_metrics.duplicate_samples / max(1, self.quality_metrics.total_samples),
            'avg_quality_score': self.quality_metrics.avg_quality_score,
            'quality_distribution': self.quality_metrics.get_quality_distribution(),
            'quality_trend': self.get_quality_trend(),
            'top_issues': sorted(
                self.quality_metrics.issue_counts.items(),
                key=lambda x: x[1],
                reverse=True
            )[:5],
        }
    
    def reset(self) -> None:
        """重置统计"""
        self.quality_metrics = DataQualityMetrics()
        self.stage_stats.clear()
        self._quality_history.clear()
        self._sample_history.clear()


# ==================== 数据样本 ====================

@dataclass
class MultiModalSample:
    """多模态数据样本
    
    支持的模态：
    - text: 文本数据
    - image: 图像数据（np.ndarray或torch.Tensor）
    - audio: 音频数据
    - video: 视频数据
    - time_series: 时序数据
    """
    sample_id: str
    modalities: Dict[str, Any] = field(default_factory=dict)  # {modality: data}
    metadata: Dict[str, Any] = field(default_factory=dict)
    
    # 质量标记
    is_valid: bool = True
    quality_score: float = 1.0
    quality_level: QualityLevel = QualityLevel.EXCELLENT
    issues: List[str] = field(default_factory=list)
    
    # 去重标记
    content_hash: Optional[str] = None
    is_duplicate: bool = False
    duplicate_of: Optional[str] = None
    
    # 处理标记
    processing_stage: ProcessingStage = ProcessingStage.RAW
    processed_time: Optional[float] = None
    
    # 来源信息
    source: Optional[DataSourceType] = None
    source_path: Optional[str] = None
    
    def add_modality(self, modality: str, data: Any) -> None:
        """添加模态数据"""
        self.modalities[modality] = data
    
    def remove_modality(self, modality: str) -> None:
        """移除模态数据"""
        if modality in self.modalities:
            del self.modalities[modality]
    
    def has_modality(self, modality: str) -> bool:
        """检查是否有指定模态"""
        return modality in self.modalities
    
    def get_modality_types(self) -> List[str]:
        """获取所有模态类型"""
        return list(self.modalities.keys())
    
    def add_issue(self, issue: str) -> None:
        """添加问题"""
        if issue not in self.issues:
            self.issues.append(issue)
            self.quality_score = max(0.0, self.quality_score - 0.1)
            self._update_quality_level()
    
    def _update_quality_level(self) -> None:
        """更新质量等级"""
        if self.quality_score >= 0.9:
            self.quality_level = QualityLevel.EXCELLENT
        elif self.quality_score >= 0.7:
            self.quality_level = QualityLevel.GOOD
        elif self.quality_score >= 0.5:
            self.quality_level = QualityLevel.FAIR
        else:
            self.quality_level = QualityLevel.POOR
    
    def mark_as_duplicate(self, original_id: str) -> None:
        """标记为重复"""
        self.is_duplicate = True
        self.duplicate_of = original_id
        self.is_valid = False
    
    def set_processing_stage(self, stage: ProcessingStage) -> None:
        """设置处理阶段"""
        self.processing_stage = stage
        self.processed_time = time.time()
    
    def to_dict(self) -> Dict[str, Any]:
        """转换为字典"""
        return {
            'sample_id': self.sample_id,
            'modalities': {
                k: str(v) if not isinstance(v, (str, int, float, bool, list, dict)) else v
                for k, v in self.modalities.items()
            },
            'metadata': self.metadata,
            'is_valid': self.is_valid,
            'quality_score': self.quality_score,
            'quality_level': self.quality_level.value,
            'issues': self.issues,
            'is_duplicate': self.is_duplicate,
            'processing_stage': self.processing_stage.value,
        }
    
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'MultiModalSample':
        """从字典创建"""
        sample = cls(
            sample_id=data['sample_id'],
            modalities=data.get('modalities', {}),
            metadata=data.get('metadata', {}),
        )
        sample.is_valid = data.get('is_valid', True)
        sample.quality_score = data.get('quality_score', 1.0)
        sample.issues = data.get('issues', [])
        sample.is_duplicate = data.get('is_duplicate', False)
        
        if 'quality_level' in data:
            sample.quality_level = QualityLevel(data['quality_level'])
        if 'processing_stage' in data:
            sample.processing_stage = ProcessingStage(data['processing_stage'])
        
        return sample
    
    def validate(self) -> Tuple[bool, List[str]]:
        """验证样本完整性"""
        errors = []
        
        if not self.sample_id:
            errors.append("Missing sample_id")
        
        if not self.modalities:
            errors.append("No modalities present")
        
        # 检查模态数据是否为空
        for modality, data in self.modalities.items():
            if data is None:
                errors.append(f"Modality {modality} is None")
            elif isinstance(data, str) and not data.strip():
                errors.append(f"Modality {modality} is empty string")
        
        return len(errors) == 0, errors
    
    def clone(self, new_id: Optional[str] = None) -> 'MultiModalSample':
        """克隆样本"""
        return MultiModalSample(
            sample_id=new_id or f"{self.sample_id}_clone",
            modalities=self.modalities.copy(),
            metadata=self.metadata.copy(),
            is_valid=self.is_valid,
            quality_score=self.quality_score,
            quality_level=self.quality_level,
            issues=self.issues.copy(),
        )
    
    def __repr__(self) -> str:
        modalities_str = ', '.join(self.modalities.keys())
        return f"MultiModalSample(id={self.sample_id}, modalities=[{modalities_str}], quality={self.quality_score:.2f})"


# ==================== 数据去重 ====================

class DataDeduplicator:
    """数据去重器
    
    支持多种去重方法：
    - perceptual_hash: 感知哈希（适用于图像）
    - minhash: MinHash（适用于文本）
    - simhash: SimHash（适用于文本）
    - content_hash: 内容哈希（通用）
    """
    
    def __init__(self, config: DataDeduplicationConfig):
        self.config = config
        self.seen_hashes: Set[str] = set()
        self.hash_to_samples: Dict[str, List[str]] = defaultdict(list)
        
        # 统计信息
        self._total_checked = 0
        self._duplicates_found = 0
        self._hash_collisions = 0
        
        # 哈希缓存（用于大规模数据）
        self._hash_cache: Optional[Dict[str, str]] = None
        if config.cache_size > 0:
            self._hash_cache = {}
    
    def compute_hash(self, sample: MultiModalSample) -> str:
        """计算样本哈希"""
        # 检查缓存
        if self._hash_cache is not None and sample.sample_id in self._hash_cache:
            return self._hash_cache[sample.sample_id]
        
        if self.config.method == "perceptual_hash":
            hash_value = self._perceptual_hash(sample)
        elif self.config.method == "minhash":
            hash_value = self._minhash(sample)
        elif self.config.method == "simhash":
            hash_value = self._simhash(sample)
        else:
            hash_value = self._content_hash(sample)
        
        # 存入缓存
        if self._hash_cache is not None:
            if len(self._hash_cache) < self.config.cache_size:
                self._hash_cache[sample.sample_id] = hash_value
        
        return hash_value
    
    def _perceptual_hash(self, sample: MultiModalSample) -> str:
        """感知哈希（主要用于图像）"""
        hashes = []
        
        # 图像感知哈希
        if 'image' in sample.modalities:
            image = sample.modalities['image']
            if isinstance(image, np.ndarray):
                # 简化的pHash实现
                # 1. 缩放到固定大小
                # 2. 转灰度
                # 3. DCT变换
                # 4. 取低频分量
                # 5. 二值化生成哈希
                h = hashlib.md5(image.tobytes()).hexdigest()
                hashes.append(f"img:{h[:16]}")
            elif isinstance(image, torch.Tensor):
                h = hashlib.md5(image.numpy().tobytes()).hexdigest()
                hashes.append(f"img:{h[:16]}")
        
        # 文本哈希
        if 'text' in sample.modalities:
            text = str(sample.modalities['text'])
            h = hashlib.md5(text.encode()).hexdigest()
            hashes.append(f"txt:{h[:16]}")
        
        return "|".join(hashes) if hashes else hashlib.md5(
            json.dumps(sample.modalities, default=str).encode()
        ).hexdigest()
    
    def _minhash(self, sample: MultiModalSample) -> str:
        """MinHash（主要用于文本）"""
        # 简化实现：使用多个哈希函数
        text = ""
        for modality, data in sample.modalities.items():
            if isinstance(data, str):
                text += data
            elif isinstance(data, list):
                text += " ".join(str(x) for x in data)
        
        # 生成n-gram
        ngrams = set()
        words = text.split()
        for i in range(len(words) - 2):
            ngrams.add(" ".join(words[i:i+3]))
        
        # 计算MinHash签名
        min_hashes = []
        for seed in range(self.config.num_perm):
            min_hash = float('inf')
            for ngram in ngrams:
                h = hash(ngram + str(seed))
                if h < min_hash:
                    min_hash = h
            min_hashes.append(min_hash)
        
        # 转换为字符串哈希
        return hashlib.md5(str(min_hashes).encode()).hexdigest()
    
    def _simhash(self, sample: MultiModalSample) -> str:
        """SimHash"""
        # 简化实现
        text = ""
        for modality, data in sample.modalities.items():
            if isinstance(data, str):
                text += data
        
        if not text:
            return self._content_hash(sample)
        
        # 分词并计算特征向量
        words = text.split()
        v = [0] * 64
        
        for word in words:
            h = int(hashlib.md5(word.encode()).hexdigest(), 16)
            for i in range(64):
                if h & (1 << i):
                    v[i] += 1
                else:
                    v[i] -= 1
        
        # 生成SimHash
        simhash = 0
        for i in range(64):
            if v[i] > 0:
                simhash |= (1 << i)
        
        return format(simhash, '016x')
    
    def _content_hash(self, sample: MultiModalSample) -> str:
        """内容哈希"""
        content = json.dumps(sample.modalities, default=str, sort_keys=True)
        return hashlib.sha256(content.encode()).hexdigest()
    
    def is_duplicate(self, sample: MultiModalSample) -> bool:
        """检查是否重复"""
        self._total_checked += 1
        
        content_hash = self.compute_hash(sample)
        sample.content_hash = content_hash
        
        if content_hash in self.seen_hashes:
            sample.is_duplicate = True
            # 找到原始样本ID
            original_samples = self.hash_to_samples.get(content_hash, [])
            if original_samples:
                sample.mark_as_duplicate(original_samples[0])
            
            self._duplicates_found += 1
            
            # 检查哈希碰撞
            if len(self.hash_to_samples[content_hash]) > 1:
                self._hash_collisions += 1
            
            return True
        
        self.seen_hashes.add(content_hash)
        self.hash_to_samples[content_hash].append(sample.sample_id)
        return False
    
    def deduplicate_batch(self, samples: List[MultiModalSample]) -> List[MultiModalSample]:
        """批量去重"""
        unique_samples = []
        for sample in samples:
            if not self.is_duplicate(sample):
                unique_samples.append(sample)
        
        logger.info(
            f"Deduplicated: {len(samples)} -> {len(unique_samples)} "
            f"({self._duplicates_found} duplicates, {self._hash_collisions} collisions)"
        )
        return unique_samples
    
    def get_duplicate_groups(self) -> List[List[str]]:
        """获取重复样本组"""
        groups = []
        for hash_value, sample_ids in self.hash_to_samples.items():
            if len(sample_ids) > 1:
                groups.append(sample_ids)
        return groups
    
    def get_statistics(self) -> Dict[str, Any]:
        """获取去重统计"""
        return {
            'total_checked': self._total_checked,
            'duplicates_found': self._duplicates_found,
            'unique_samples': len(self.seen_hashes),
            'duplicate_rate': self._duplicates_found / max(1, self._total_checked),
            'hash_collisions': self._hash_collisions,
            'collision_rate': self._hash_collisions / max(1, len(self.seen_hashes)),
            'method': self.config.method,
        }
    
    def save_index(self, path: str) -> None:
        """保存去重索引"""
        data = {
            'seen_hashes': list(self.seen_hashes),
            'hash_to_samples': dict(self.hash_to_samples),
            'statistics': self.get_statistics(),
        }
        with open(path, 'wb') as f:
            pickle.dump(data, f)
        logger.info(f"Deduplication index saved to {path}")
    
    def load_index(self, path: str) -> None:
        """加载去重索引"""
        with open(path, 'rb') as f:
            data = pickle.load(f)
        
        self.seen_hashes = set(data['seen_hashes'])
        self.hash_to_samples = defaultdict(list, data['hash_to_samples'])
        
        if 'statistics' in data:
            stats = data['statistics']
            self._total_checked = stats.get('total_checked', 0)
            self._duplicates_found = stats.get('duplicates_found', 0)
            self._hash_collisions = stats.get('hash_collisions', 0)
        
        logger.info(f"Deduplication index loaded from {path}")
    
    def clear(self) -> None:
        """清空去重索引"""
        self.seen_hashes.clear()
        self.hash_to_samples.clear()
        if self._hash_cache is not None:
            self._hash_cache.clear()
        self._total_checked = 0
        self._duplicates_found = 0
        self._hash_collisions = 0
    
    def estimate_similarity(self, hash1: str, hash2: str) -> float:
        """估算两个哈希的相似度（仅对SimHash有效）"""
        if self.config.method != "simhash":
            return 1.0 if hash1 == hash2 else 0.0
        
        # 计算汉明距离
        try:
            val1 = int(hash1, 16)
            val2 = int(hash2, 16)
            xor = val1 ^ val2
            distance = bin(xor).count('1')
            # 转换为相似度
            return 1.0 - (distance / 64.0)
        except:
            return 0.0


# ==================== 数据过滤 ====================

class DataFilter:
    """数据过滤器"""
    
    def __init__(self, config: DataFilterConfig):
        self.config = config
        self.nsfw_detector = None
        self.language_detector = None
    
    def filter(self, sample: MultiModalSample) -> bool:
        """过滤样本，返回是否保留"""
        issues = []
        
        # 文本长度检查
        if 'text' in sample.modalities:
            text = str(sample.modalities['text'])
            if len(text) < self.config.min_text_length:
                issues.append(f"Text too short: {len(text)}")
            if len(text) > self.config.max_text_length:
                issues.append(f"Text too long: {len(text)}")
        
        # 图像尺寸检查
        if 'image' in sample.modalities:
            image = sample.modalities['image']
            if isinstance(image, np.ndarray):
                h, w = image.shape[:2]
            elif isinstance(image, torch.Tensor):
                h, w = image.shape[-2:]
            else:
                h, w = self.config.min_image_size, self.config.min_image_size
            
            if h < self.config.min_image_size or w < self.config.min_image_size:
                issues.append(f"Image too small: {h}x{w}")
            if h > self.config.max_image_size or w > self.config.max_image_size:
                issues.append(f"Image too large: {h}x{w}")
        
        # NSFW过滤（简化实现）
        if self.config.nsfw_filter:
            nsfw_score = self._check_nsfw(sample)
            if nsfw_score > self.config.nsfw_threshold:
                issues.append(f"NSFW content detected: {nsfw_score:.2f}")
        
        # 模态一致性检查
        if self.config.consistency_check:
            consistency_score = self._check_consistency(sample)
            if consistency_score < self.config.consistency_threshold:
                issues.append(f"Low consistency: {consistency_score:.2f}")
        
        # 语言过滤
        if self.config.language_filter and 'text' in sample.modalities:
            lang = self._detect_language(sample.modalities['text'])
            if lang not in self.config.language_filter:
                issues.append(f"Language mismatch: {lang}")
        
        sample.issues = issues
        sample.is_valid = len(issues) == 0
        sample.quality_score = 1.0 - len(issues) * 0.2
        
        return sample.is_valid
    
    def _check_nsfw(self, sample: MultiModalSample) -> float:
        """NSFW检测（简化实现）"""
        # 实际应使用专门的NSFW检测模型
        return 0.0
    
    def _check_consistency(self, sample: MultiModalSample) -> float:
        """模态一致性检查"""
        # 检查图文匹配度等
        # 实际应使用CLIP等模型计算相似度
        if len(sample.modalities) < 2:
            return 1.0
        
        # 简化：检查是否都有内容
        has_content = [
            bool(data) for data in sample.modalities.values()
        ]
        return sum(has_content) / len(has_content)
    
    def _detect_language(self, text: str) -> str:
        """语言检测"""
        # 简化实现
        if any('\u4e00' <= char <= '\u9fff' for char in text):
            return 'zh'
        return 'en'
    
    def filter_batch(self, samples: List[MultiModalSample]) -> List[MultiModalSample]:
        """批量过滤"""
        valid_samples = []
        for sample in samples:
            if self.filter(sample):
                valid_samples.append(sample)
        
        logger.info(f"Filtered: {len(samples)} -> {len(valid_samples)}")
        return valid_samples


# ==================== 数据增强 ====================

class DataAugmentor:
    """数据增强器"""
    
    def __init__(self, config: DataAugmentationConfig):
        self.config = config
    
    def augment(self, sample: MultiModalSample) -> List[MultiModalSample]:
        """数据增强"""
        augmented = [sample]  # 原始样本
        
        # 图像增强
        if 'image' in sample.modalities and isinstance(
            sample.modalities['image'], (np.ndarray, torch.Tensor)
        ):
            augmented.extend(self._augment_image(sample))
        
        # 文本增强
        if 'text' in sample.modalities and self.config.text_synonym_replace:
            augmented.extend(self._augment_text(sample))
        
        return augmented
    
    def _augment_image(self, sample: MultiModalSample) -> List[MultiModalSample]:
        """图像增强"""
        augmented = []
        image = sample.modalities['image']
        
        # 水平翻转
        if self.config.image_flip:
            new_sample = MultiModalSample(
                sample_id=f"{sample.sample_id}_flip",
                modalities={**sample.modalities},
                metadata={**sample.metadata, 'augment': 'flip'}
            )
            if isinstance(image, np.ndarray):
                new_sample.modalities['image'] = np.fliplr(image)
            elif isinstance(image, torch.Tensor):
                new_sample.modalities['image'] = torch.flip(image, dims=[-1])
            augmented.append(new_sample)
        
        return augmented
    
    def _augment_text(self, sample: MultiModalSample) -> List[MultiModalSample]:
        """文本增强"""
        # 简化实现
        return []
    
    def augment_batch(self, samples: List[MultiModalSample]) -> List[MultiModalSample]:
        """批量增强"""
        all_augmented = []
        for sample in samples:
            all_augmented.extend(self.augment(sample))
        
        logger.info(f"Augmented: {len(samples)} -> {len(all_augmented)}")
        return all_augmented


# ==================== 数据管道 ====================

class MultiModalDataPipeline:
    """多模态数据处理管道"""
    
    def __init__(self, config: DataEngineeringConfig):
        self.config = config
        
        # 初始化各处理器
        self.deduplicator = DataDeduplicator(config.deduplication)
        self.filter = DataFilter(config.filtering)
        self.augmentor = DataAugmentor(config.augmentation)
        
        # 统计
        self.stats = {
            'total_input': 0,
            'after_dedup': 0,
            'after_filter': 0,
            'after_augment': 0
        }
    
    def process(self, samples: List[MultiModalSample]) -> List[MultiModalSample]:
        """处理数据管道"""
        self.stats['total_input'] = len(samples)
        
        # 1. 去重
        if self.config.deduplication.enabled:
            samples = self.deduplicator.deduplicate_batch(samples)
        self.stats['after_dedup'] = len(samples)
        
        # 2. 过滤
        if self.config.filtering.enabled:
            samples = self.filter.filter_batch(samples)
        self.stats['after_filter'] = len(samples)
        
        # 3. 增强
        if self.config.augmentation.enabled:
            samples = self.augmentor.augment_batch(samples)
        self.stats['after_augment'] = len(samples)
        
        logger.info(f"Pipeline stats: {self.stats}")
        
        return samples
    
    def process_stream(self, 
                      sample_iterator: Iterator[MultiModalSample],
                      batch_size: int = 1000) -> Iterator[MultiModalSample]:
        """流式处理"""
        batch = []
        
        for sample in sample_iterator:
            batch.append(sample)
            
            if len(batch) >= batch_size:
                processed = self.process(batch)
                for s in processed:
                    yield s
                batch = []
        
        # 处理剩余样本
        if batch:
            processed = self.process(batch)
            for s in processed:
                yield s
    
    def get_statistics(self) -> Dict[str, Any]:
        """获取统计信息"""
        return {
            **self.stats,
            'dedup_rate': 1 - self.stats['after_dedup'] / max(self.stats['total_input'], 1),
            'filter_rate': 1 - self.stats['after_filter'] / max(self.stats['after_dedup'], 1),
            'augment_rate': self.stats['after_augment'] / max(self.stats['after_filter'], 1) - 1
        }


# ==================== 数据集构建器 ====================

class MultiModalDatasetBuilder:
    """多模态数据集构建器"""
    
    def __init__(self, config: DataEngineeringConfig):
        self.config = config
        self.pipeline = MultiModalDataPipeline(config)
        self.samples: List[MultiModalSample] = []
    
    def add_samples(self, samples: List[MultiModalSample]):
        """添加样本"""
        self.samples.extend(samples)
    
    def add_from_source(self, source: DataSourceType, data_path: str):
        """从数据源添加"""
        logger.info(f"Loading from {source.value}: {data_path}")
        
        if source == DataSourceType.WEB_IMAGE_TEXT:
            self._load_web_image_text(data_path)
        elif source == DataSourceType.VIDEO_ASR:
            self._load_video_asr(data_path)
        elif source == DataSourceType.OCR_DOCUMENT:
            self._load_ocr_document(data_path)
        # ... 其他数据源
    
    def _load_web_image_text(self, data_path: str):
        """加载图文网页数据"""
        # 实现具体的加载逻辑
        pass
    
    def _load_video_asr(self, data_path: str):
        """加载视频+ASR数据"""
        pass
    
    def _load_ocr_document(self, data_path: str):
        """加载OCR文档数据"""
        pass
    
    def build(self) -> List[MultiModalSample]:
        """构建数据集"""
        logger.info(f"Building dataset with {len(self.samples)} samples")
        
        # 通过管道处理
        processed_samples = self.pipeline.process(self.samples)
        
        logger.info(f"Dataset built: {len(processed_samples)} samples")
        return processed_samples
    
    def save(self, output_path: str):
        """保存数据集"""
        processed_samples = self.build()
        
        os.makedirs(output_path, exist_ok=True)
        
        # 保存样本
        for i, sample in enumerate(processed_samples):
            sample_path = os.path.join(output_path, f"sample_{i:08d}.json")
            with open(sample_path, 'w', encoding='utf-8') as f:
                json.dump({
                    'sample_id': sample.sample_id,
                    'modalities': {
                        k: str(v) if not isinstance(v, (str, int, float, bool, list, dict)) else v
                        for k, v in sample.modalities.items()
                    },
                    'metadata': sample.metadata,
                    'quality_score': sample.quality_score
                }, f, ensure_ascii=False, indent=2)
        
        # 保存统计信息
        stats_path = os.path.join(output_path, "stats.json")
        with open(stats_path, 'w', encoding='utf-8') as f:
            json.dump(self.pipeline.get_statistics(), f, indent=2)
        
        logger.info(f"Dataset saved to {output_path}")

